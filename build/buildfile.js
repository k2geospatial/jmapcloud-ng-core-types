...
if (fs.existsSync(join(__dirname, "env-config.js"))) {
  ...
  const toCheckVariables = ["COPY_DIR", "SOURCE_DIRS"]
...
function generateTypedoc() {
  ...
  execSync(
    `cat ${join(__dirname, "../public/core.d.ts")} ${join(__dirname, "../public/jmap/*ts")} > ${join(
      __dirname,
      "./JMap.d.ts"
    )}`,
    {
...
function copyTypesToOtherProjects() {
  ...
  for (const sourceDir of sourceDirs) {
    console.info(`Copy core type files in "${join(process.env.COPY_DIR, `${sourceDir}`)}"`)

    fs.cpSync(
      join(ROOT_DIR, "public/"),
      join(process.env.COPY_DIR, `${sourceDir}/node_modules/jmapcloud-ng-core-types/public/`),
      { recursive: true, force: true }
    )
    fs.cpSync(
      join(ROOT_DIR, "index.ts"),
      join(process.env.COPY_DIR, `${sourceDir}/node_modules/jmapcloud-ng-core-types/index.ts`),
      { recursive: true, force: true }
    )
    fs.cpSync(
      join(ROOT_DIR, "all-enums.ts"),
      join(process.env.COPY_DIR, `${sourceDir}/node_modules/jmapcloud-ng-core-types/all-enums.ts`),
      { recursive: true, force: true }
    )
  }
}
